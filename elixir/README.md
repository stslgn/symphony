...
canonical repository name and provider-stable repository id, PR, approved base ref and head SHA,
channel, executor, and workflow generation. Claims use unique ids and monotonically increasing
fencing tokens, never expire by time, and remain recoverable only by the same owner through the
closed recovery state machine. The claim response returns a one-time owner capability; callers must
keep it out of logs and provide it for every transition, recovery, and provider mutation. Only its
SHA-256 digest is persisted, and neither the digest nor the capability is exposed by `history`.
An active session claim makes a matching native runner approval record a bounded conflict and fail
closed without releasing the wait or entering `Merging`.
...
`claim` accepts the full binding, including `repository_id`. `transition` accepts `claim_id`,
`fencing_token`, `owner_capability`, `target_state`, and bounded evidence; `recover` accepts the exact
claim/fence/capability, a closed-table action, and evidence whose digest is recomputed over the
accepted evidence fields and the recovery action. Every successful command emits one JSON object.
Malformed or oversized ledgers, conflicting claims, stale fences, owner mismatches, workflow drift,
and an existing operation lock all fail closed. History is capped to the latest 1,000 events, while
the internal ledger is capped at 20,000 events or 8 MiB. An operation lock is not removed by age;
recovery requires first proving that no process still owns the logs root.
...
The provider mutation boundary reloads the current claim under the merge-lane operation lock and
requires its exact owner capability, current fencing token, and `executing` state. The current PR
snapshot must match the approved stable repository id, canonical repository name, PR, base, and head
tuple. The boundary additionally requires provider capability evidence for atomic expected-head
enforcement plus either atomic base-ref enforcement or proved repository controls that make base
retargeting impossible. Head-only GitHub matching is rejected as
