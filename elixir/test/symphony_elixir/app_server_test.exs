defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server handles MCP server elicitations when approval policy is never" do
    test_root =
      Path.join(
        System.tmp_dir!(),
    ...
    try do
      ...
      while IFS= read -r line; do
        ...
        case "$count" in
          ...
          4)
            ...
            printf '%s\\n' '{"id":113,"method":"mcpServer/elicitation/request","params":{"serverName":"linear","threadId":"thread-720","turnId":"turn-720","mode":"form","message":"Allow the Linear write?","requestedSchema":{"type":"object","properties":{}},"_meta":{"codex_approval_kind":"mcp_tool_call"}}}'
            ;;
          5)
      ...
      issue = %Issue{
        ...
        identifier: "MT-720",
        title: "Handle MCP server elicitations",
        description: "Ensure MCP elicitations follow the non-interactive approval policy",
        state: "In Progress",
        url: "https://example.org/issues/MT-720",
        labels: ["backend"]
      }

      assert {:ok, _result} = AppServer.run(workspace, "Handle MCP server elicitations", issue)
      ...
      assert Enum.any?(responses, fn payload ->
               payload["id"] == 113 and get_in(payload, ["result", "action"]) == "accept" and
  ...
  test "app server treats runtime error notifications as terminal turn errors" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-runtime-error-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      issue = %Issue{
        id: "issue-runtime-error",
        identifier: "MT-95",
        title: "Runtime error notification",
        description: "Ensure runtime errors fail fast instead of burning continuation turns",
        state: "In Progress",
